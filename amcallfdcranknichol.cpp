...
double AmCallFDCrankNichLN( double S, double K, double r, double sigma, double time, int no_S_steps, int no_t_steps) {
    int N=no_t_steps;
    ...
    for (int m=1;m<M;++m){
    	 Cup = max(0.0,exp(X_values[m-1])-K);
    	 Cm = max(0.0,exp(X_values[m])-K);
    	 Cdown = max(0.0,exp(X_values[m+1])-K);
    	 B[m]= -Pu*Cup - (Pm-2)*Cm - Pd*Cdown;
    };
    B[M]=0; B[0]=exp(X_values[M])-exp(X_values[M-1]);
    ...
    for(int t=N-1;t>0;--t) {
        ...
        F = inverse(A)*B;
        for (int m=1;m<M;++m) { 
            F[m] = max(F[m], exp(X_values[m])-K);
        };
    };
    cout << endl;
    ofstream myfile; myfile.precision(15); myfile.open("amcallcranknich.csv");
